...
static T_S32 anyka_fs_write(T_S32 hFile, T_pVOID buf, T_S32 size)
{
	...
	dbg_printf("hFile===%d  size==%d\n",hFile,size);
	static data_sync_t * prev_data = NULL;
	data_sync_t * ofsset_data = NULL;
	if(NULL == prev_data)
	{
		prev_data = calloc(1,sizeof(data_sync_t));	
		prev_data->fd = hFile;
		prev_data->length = 0;
		prev_data->need_seek = -1;
		prev_data->buff = calloc(1,sizeof(unsigned char)*32*1024);
	}
	
	if(need_seek==1)
	{

		if((NULL != prev_data) && (prev_data->length > 0) )
		{
			mux_push_async_data(prev_data);
			prev_data = NULL;
		}
		ofsset_data = calloc(1,sizeof(data_sync_t));
		ofsset_data->fd = hFile;
		ofsset_data->length = size;
		ofsset_data->buff = calloc(1,sizeof(unsigned char)*size);
		memmove(ofsset_data->buff,buf,size);
		ofsset_data->need_seek = 1;
		ofsset_data->seek_offset= offset_anyka;
		need_seek = 0;
		mux_push_async_data(ofsset_data);
		ofsset_data = NULL;
	}
	else
	{

		if(prev_data->length+size < 32*1024)
		{
			memmove(prev_data->buff+prev_data->length,buf,size);
			prev_data->length += size;
		}
		else
		{
			int size0 = 32*1024-prev_data->length;
			memmove(prev_data->buff+prev_data->length,buf,size0);
			prev_data->length += size0;
			mux_push_async_data(prev_data);
			prev_data = NULL;
			...
			prev_data->need_seek = -1;
			prev_data->buff = calloc(1,sizeof(unsigned char)*32*1024);
			memmove(prev_data->buff+prev_data->length,buf+size0,size1);
			prev_data->length += size1;
